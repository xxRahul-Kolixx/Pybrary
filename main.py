...
if __name__ == "__main__":
    if os.environ.get("ALLOW_MAIN_EXECUTION") != "true":
...
def add_record():
	...
	if not bk_id.get() or not author_name.get() or not bk_name.get():
		mb.showerror('Incomplete Data', 'Please fill in  Book ID, Author Name, and Book Name fields.')
		return
...
# Variables
lf_bg = '#EBEF95'  # Left Frame Background Color
rtf_bg = '#EFD595'  # Right Top Frame Background Color
rbf_bg = '#EFB495'  # Right Bottom Frame Background Color
btn_hlb_bg = '#EF9595'  # Background color for Head Labels and Buttons
...
root.iconbitmap('read.ico')
root.resizable(0, 0)

Label(root, text='PYBRARY', font=("Noto Sans CJK TC", 15, 'bold'), bg=btn_hlb_bg, fg='White').pack(side=TOP, fill=X)
...
left_frame.place(relx=0.7, y=30, relheight=0.96, relwidth=0.3)

RT_frame = Frame(root, bg=rtf_bg)
RT_frame.place(relx=0, y=30, relheight=0.2, relwidth=0.7)

RB_frame = Frame(root)
RB_frame.place(relx=0, rely=0.24, relheight=0.785, relwidth=0.7)
...
# Right Bottom Frame
Label(RB_frame, text='BOOKS', bg=rbf_bg, font=("Noto Sans CJK TC", 15, 'bold')).pack(side=TOP, fill=X)
...
tree.column('#5', width=135, stretch=NO)
